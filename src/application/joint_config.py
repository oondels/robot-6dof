...
class JointConfig:
    ...
    def _calculate_position(
        ...
        ### realiza o cálculo interno da posição do servo com base no ângulo fornecido, levando em consideração a posição zero e a direção da junta.
        ### Conversor de angulo para posicao do servo (counts)
        return round(
            self.zero_position + self.direction * angle * STEPS_PER_REVOLUTION / 360.0
        )

    def _validate_mapped_positions(self) -> None:
        min_position = self._calculate_position(self.min_angle)

        max_position = self._calculate_position(self.max_angle)
    ...
    def tolerance_counts(self) -> int:
        ### Calcula a tolerância em counts com base na tolerância em graus, garantindo que seja pelo menos 1 count.
        return max(
            1,
    ...
    def angle_to_position(self, angle: float) -> int:
            ...
            # TODO: Verificar se usar uma funcao propria para verificar valor dentro de determinado intervalo nao seria mais adequada, pois pode ser
            # realizado ajustes nesse momento, ou usadado para dados
            ...
            return self._calculate_position(angle)
    ...
    def position_to_angle(self, position: int) -> float:
        ...
        first_limit = self._calculate_position(self.min_angle)
        second_limit = self._calculate_position(self.max_angle)
        ...
        if not min_position <= position <= max_position:
            # TODO: Verificar se lancar esse erro é realmente necessário ou se emitir um sinal para calibragem ou algo do tipo seria mais adequado., 
            raise ValueError(
