...
def validate_servo_id(servo_id: int) -> None:
    ...
    if not MIN_SERVO_ID <= servo_id <= MAX_SERVO_ID:
        raise ValueError(
            f"servo_id deve estar entre {MIN_SERVO_ID} e {MAX_SERVO_ID}"
        )
...
def run_reader(
    ...
    while True:
        command = input_fn(
            "Pressione Enter para ler os counts ou digite q para sair: "
        )
...
def main() -> None:
    ...
    try:
        ...
        if not port.setBaudRate(args.baudrate):
            raise RuntimeError(
                f"Erro configurando baudrate {args.baudrate}"
            )
